...
struct urltestcase urllist[] = {
  {"http://example.com/hello/../here",
   "http://example.com/hello/../here",
   CURLURL_PATH_AS_IS, 0, CURLURLE_OK},
  {"http://example.com/hello/../here",
...
int test(char *URL)
{
  ...
  for(i = 0; rellist[i].in; i++) {
    char *url = NULL;
    urlp = NULL;
    rc = curl_url((char *)rellist[i].in, &urlp, rellist[i].urlflags);
    if(!rc) {
      rc = curl_url_set(urlp, CURLUPART_URL, (char *)rellist[i].set,
                        rellist[i].setflags);

      rc = curl_url_get(urlp, CURLUPART_URL, &url, 0);

      if(checkurl(url, rellist[i].out)) {
        return 3;
      }
      curl_free(url);
    }
    else if(rc != rellist[i].ucode) {
      fprintf(stderr, "in: %s\nreturned %d (expected %d)\n", rellist[i].in,
              (int)rc, rellist[i].ucode);
      return 4;
    }
